...
1. Introduce commands as separate classes and bind them to AutoFac (you can use Marto's slick way)

## Step by step guide

- Install AutoFac from NuGet
- Setup a **separate config class** that will handle autofac related configurations. It could look something something like this:
...
internal sealed class AutofacConfig
{
    ...
    {
        builder.RegisterType<FurnitureManufacturerEngine>().As<IEngine>();
        // Write the rest of your bindings... (if needed)
    }
}
```

**HINT** - Copy/pasting code is fine, as long as you **truely understand what it's doing and why**. Take some time to think about why did I implement that class as **sealed** and why did I introduce several methods into it.
...
- Let's fix up the engine a little bit. Pull each command into a **separate class**!
- You could introduce a **separate class** to hold these dictionaries from the Engine
